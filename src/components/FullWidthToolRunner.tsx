...
import { 
  ...
  ChevronLeft, ChevronRight, Calculator, Copy, Layers, ZoomIn, ZoomOut, RotateCw, Code, Music, Video, Image as ImageIcon, FileSpreadsheet
} from 'lucide-react';
...
export function resolveToolConfig(tool: AITool) {
  ...
  if (name.includes('merge') || id.includes('merge')) {
    showConvertDropdown = false; formatOptions = ['PDF Document (.pdf)']; defaultExt = 'pdf';
    uploadLabel = 'Upload PDF Files to Merge (Select Multiple)'; allowMultiple = true; actionButtonText = 'Merge PDFs';
  } else if (name.includes('split') || id.includes('split')) {
    showConvertDropdown = false; formatOptions = ['PDF Document (.pdf)']; defaultExt = 'pdf'; uploadLabel = 'Upload PDF to Split'; actionButtonText = 'Split PDF';
  } else if (name.includes('compress') || name.includes('shrink') || id.includes('compress')) {
    showConvertDropdown = false; formatOptions = ['PDF Document (.pdf)']; defaultExt = 'pdf'; uploadLabel = 'Upload PDF to Compress'; actionButtonText = 'Compress PDF';
  } else if (name.includes('rotate') || id.includes('rotate')) {
    showConvertDropdown = false; formatOptions = ['PDF Document (.pdf)']; defaultExt = 'pdf'; uploadLabel = 'Upload PDF to Rotate'; actionButtonText = 'Rotate PDF';
  } else if (name.includes('delete') || id.includes('delete')) {
    showConvertDropdown = false; formatOptions = ['PDF Document (.pdf)']; defaultExt = 'pdf'; uploadLabel = 'Upload PDF Document'; actionButtonText = 'Delete Pages';
    ...
    showConvertDropdown = true; formatOptions = ['PowerPoint Presentation (.pptx)']; defaultExt = 'pptx'; uploadLabel = 'Upload PDF Document'; actionButtonText = 'Convert to PPT';
    ...
    showConvertDropdown = false; formatOptions = ['PDF Document (.pdf)']; defaultExt = 'pdf'; uploadLabel = 'Upload PDF Document'; actionButtonText = 'Process PDF';
  } else if (cat.includes('calc') || cat.includes('finance') || name.includes('calculator') || name.includes('emi')) {
    showConvertDropdown = false; showUpload = false; formatOptions = ['Text Report (.txt)', 'CSV Data (.csv)']; defaultExt = 'txt'; actionButtonText = 'Calculate Now';
  } else {
    showConvertDropdown = false; showUpload = false; formatOptions = ['Plain Text (.txt)']; defaultExt = 'txt'; actionButtonText = 'Execute Tool';
  }
    ...
    <div className="space-y-2 font-sans text-xs text-slate-200">
      {lines.map((line, idx) => {
        ...
        if (trimmed.startsWith('#')) {
          ...
          return <div key={idx} className="text-sm font-extrabold text-emerald-400 mt-3 mb-2 flex items-center gap-2 border-b border-white/5 pb-1">{parseInlineBold(cleanHeading)}</div>;
        }
        if (trimmed.startsWith('- ') || trimmed.startsWith('* ')) {
          const cleanBullet = trimmed.replace(/^[-*]\s*/, '');
          return <div key={idx} className="flex items-start gap-2 pl-2 py-0.5 text-slate-200 hover:bg-white/5 rounded transition-colors"><span className="text-emerald-400 font-bold">•</span><div>{parseInlineBold(cleanBullet)}</div></div>;
        }
...
export const FullWidthToolRunner: React.FC<FullWidthToolRunnerProps> = ({
  ...
  // DETERMINE ACTIVE EXTENSION FROM SELECTION
  ...
  else if (selectedFormat.toLowerCase().includes('.webp')) activeExt = 'webp';
  ...
  else if (selectedFormat.toLowerCase().includes('.xlsx')) activeExt = 'xlsx';
  else if (selectedFormat.toLowerCase().includes('.csv')) activeExt = 'csv';
  ...
  // 🚀 ORIGINAL FILE NAMING SYSTEM (E.g., 1942-land-converted.jpg)
  const handleDirectDownloadFile = async () => {
    ...
    // Check if user uploaded a file and use its original name
    if (uploadedFiles.length > 0) {
      const originalName = uploadedFiles[0].name;
      const dotIndex = originalName.lastIndexOf('.');
      if (dotIndex !== -1) {
        baseName = originalName.substring(0, dotIndex); // Strip old extension
      } else {
        baseName = originalName;
      }
    }

    const filename = `${baseName}-converted.${activeExt}`;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
          ...
          <div className="lg:col-span-4 bg-[#151517] border border-white/10 rounded-lg p-4 space-y-3">
            <div className="flex items-center justify-between pb-2 border-b border-white/10">
              <span className="text-xs font-bold uppercase text-emerald-400 flex items-center gap-1.5"><Sliders className="w-3.5 h-3.5" /> Tool Parameters</span>
            </div>
            ...
            <button onClick={handleExecute} disabled={isRunning} className="w-full py-3 bg-emerald-600 hover:bg-emerald-500 text-slate-950 font-extrabold text-xs uppercase rounded-lg flex items-center justify-center gap-2 shadow-lg shadow-emerald-600/20 transition-all cursor-pointer">
          ...
          <div className="lg:col-span-8 bg-[#151517] border border-white/10 rounded-lg p-4 min-h-[560px] flex flex-col justify-between shadow-2xl">
            <div>
              ...
              {/* CLEAN UI OUTPUT TEXT WORKSPACE */}
            ...
            <div className="mt-4 pt-3 border-t border-white/10">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                {relatedTools.map((t) => (
                  <button key={t.id} onClick={() => onSelectTool(t)} className="p-2 bg-white/5 hover:bg-emerald-600/10 border border-white/10 hover:border-emerald-500/40 rounded-lg text-left transition-all group cursor-pointer flex flex-col justify-between">
                    <span className="text-[11px] font-bold text-white group-hover:text-emerald-400 truncate">{t.name}</span>
